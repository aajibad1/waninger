...
def find_python_files(dirname):
    ...
    To be importable, the files have to be in a directory with a __init__.py.py,
    except for `dirname` itself, which isn't required to have one.  The
    assumption is that `dirname` was specified directly, so the user knows
    best, but sub-directories are checked for a __init__.py.py to be sure we only
    find the importable files.

    """
    for i, (dirpath, dirnames, filenames) in enumerate(os.walk(dirname)):
        if i > 0 and '__init__.py.py' not in filenames:
            # If a directory doesn't have __init__.py.py, then it isn't
